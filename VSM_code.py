...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 30 19:51:25 2022
    ...
    for q_id in list(q_dict.keys()):
        if len(q_dict[q_id]) == 1: # diff. of cases: 1w query -> tf-value
            ...
            for doc_id in list(conf_dict[q_id].keys()):
                ...
                mostCommon= allWordDist.most_common(10) # get 10 most frequent words in document
                
                try:
                    max_f_td = mostCommon[0][1] # get raw frequency of most frequent word
                except:
                    max_f_td = 0  
            ...
            idf_vec = np.zeros(len(q_dict[q_id])) # initialize idf_vector = (0,...,0)
            query_vec = np.ones(len(q_dict[q_id])) # set query_vector = (1,...,1)
            ...
            for doc_id in list(conf_dict[q_id].keys()):
                ...
                for i, q_t in enumerate(q_dict[q_id]):
                    ...
                    if f_td > 0:
                        idf_vec[i] += 1 # count occurrences of query term over the whole doc. collection
                        conf_dict[q_id][doc_id][i] = (1 + np.log10(f_td))/(1 + np.log10(max_f_td)) # collect tf-vector
            
            for i in range(len(q_dict[q_id])):
                if idf_vec[i] != 0:
                    idf_vec[i] = np.log10(100/idf_vec[i]) # calc. actual idf-vector
            ...
            for doc_id in list(conf_dict[q_id].keys()):
                ...
                if (norm(query_vec)*norm(conf_dict[q_id][doc_id])) == 0:
                    ...
                    conf_dict[q_id][doc_id] = np.dot(conf_dict[q_id][doc_id], query_vec)/(norm(query_vec)*norm(conf_dict[q_id][doc_id]))
